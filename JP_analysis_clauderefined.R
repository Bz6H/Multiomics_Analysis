...
# raw normalised (log) values as stored in the matrix
raw <- as.numeric(assay(gem)[idx, ])

# imputed (MAGIC-smoothed) values — what plotEmbedding displays when impute weights exist
imp <- as.numeric(imputeMatrix(
  mat           = assay(gem)[idx, , drop = FALSE],
  imputeWeights = getImputeWeights(JPproj)
))

grp <- as.character(getCellColData(JPproj)[colnames(gem), group])
df  <- data.frame(group = grp, raw = raw, imp = imp)

summ <- df %>%
  group_by(group) %>%
  summarise(
    n        = n(),
    pct_expr = 100 * mean(raw > 0),   # % cells with any detected transcript
    min      = min(raw),
    max      = max(raw),
    mean     = mean(raw),
    median   = median(raw),
    ...
    var      = var(raw),
    cv       = sd(raw) / mean(raw),
    imp_mean = mean(imp),             # same stats on the imputed layer
    imp_max  = max(imp),
    imp_p95  = quantile(imp, 0.95),
    .groups  = "drop"
  ) %>%
  arrange(group)
...
plot_list <- lapply(marker_genes, function(g) {
  plotEmbedding(JPproj, colorBy = "GeneScoreMatrix", name = g,
                embedding = "UMAP_Harmony_ATAC", plotAs = "points",
                size = 0.5, rastr = TRUE, quantCut = NULL,
                imputeWeights = getImputeWeights(JPproj)) +
...
plotEmbedding(
  ...
  name = paste0("deviations:", "TCF7"),              # specific TF motif name
