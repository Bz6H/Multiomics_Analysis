...
setwd("/Users/yuzhihuang/SOX17_predicted_sites")
...
cat("Unique peaks with SOX17 motif:", length(unique(homer_out$PositionID)), "\n")
...
cat("SOX17-linked peaks:", nrow(links), "\n")
cat("Unique predicted SOX17 target genes:", length(unique(links$geneName)), "\n")
table(links$peakType)

write.csv(
  links,
  file      = "/Users/yuzhihuang/SOX17_predicted_sites/Predicted_SOX17_regulon.csv",
  row.names = FALSE
)

cat("Saved to: /Users/yuzhihuang/SOX17_predicted_sites/Predicted_SOX17_regulon.csv\n")


peaks_multi <- read.csv("/Users/yuzhihuang/SOX17_predicted_sites/Predicted_SOX17_regulon.csv")
...
SOX17_genes <- unique(peaks_multi$geneName)
SOX17_genes <- SOX17_genes[!is.na(SOX17_genes)]
...
PAX8_SOX17_coreg <- intersect(PAX8_genes, SOX17_genes)
length(PAX8_SOX17_coreg)

dir.create(paste0(projPath, "/Validated/SOX17"), recursive = TRUE, showWarnings = FALSE)

write.table(
  PAX8_SOX17_coreg,
  file = paste0(projPath, "/Validated/SOX17/PAX8_SOX17_coregulated_genes.txt"),
...
dir.create(paste0(projPath, "/Output/SOX17_PAX8_coregulon"), recursive = TRUE, showWarnings = FALSE)
...
for (day in Timepoints) {
  ...
  gene_file <- paste0(projPath, "/Validated/SOX17/PAX8_SOX17_coregulated_genes.txt")
  ...
  pdf(paste0(projPath, "/Output/SOX17_PAX8_coregulon/", day, "_SOX17+PAX8_GOBP_Dotplot.pdf"),
      width = 16, height = 12)
  print(dotplot(go_simple_BP, showCategory = 30) +
          ggtitle(paste(day, "SOX17/PAX8 Target Genes: Biological Processes")))
  dev.off()
  
  pdf(paste0(projPath, "/Output/SOX17_PAX8_coregulon/", day, "_SOX17+PAX8_GOMF_Dotplot.pdf"),
      width = 16, height = 12)
  print(dotplot(go_simple_MF, showCategory = 30) +
          ggtitle(paste(day, "SOX17/PAX8 Target Genes: Molecular Functions")))
  dev.off()
  
  pdf(paste0(projPath, "/Output/SOX17_PAX8_coregulon/", day, "_SOX17+PAX8_GOCC_Dotplot.pdf"),
      width = 16, height = 8)
  print(dotplot(go_simple_CC, showCategory = 20) +
          ggtitle(paste(day, "SOX17/PAX8 Target Genes: Cellular Components")))
  dev.off()
  
  write.csv(as.data.frame(go_simple_BP),
            paste0(projPath, "/Output/SOX17_PAX8_coregulon/", day, "_SOX17+PAX8_GOBP_Results.csv"),
            row.names = FALSE)
  
  write.csv(as.data.frame(go_simple_MF),
            paste0(projPath, "/Output/SOX17_PAX8_coregulon/", day, "_SOX17+PAX8_GOMF_Results.csv"),
            row.names = FALSE)
  
  write.csv(as.data.frame(go_simple_CC),
            paste0(projPath, "/Output/SOX17_PAX8_coregulon/", day, "_SOX17+PAX8_GOCC_Results.csv"),
            row.names = FALSE)
